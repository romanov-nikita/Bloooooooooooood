...
class Q_in():
    ...
    def Q_der1(self, t):
        t += 0.2
        return self.SV / self.A() * (self.n * self.w() * cos(self.w() * t) * sin(self.w() * t) ** (self.n - 1) * cos(self.w() * t - self.phi) - self.w() * sin(self.w() * t) ** self.n * sin(self.w() * t - self.phi))
    
    def Q_der2(self, t):
        t += 0.2
        return self.SV / self.A() * (-self.w() ** 2) * sin(self.w() * t) ** (self.n - 2) * ( (1 - self.n) * self.n * cos(self.w() * t - self.phi) + (self.n + 1) * sin(self.w() * t) ** 2 * cos(self.w() * t - self.phi) + 2 * self.n * sin(self.w() * t) * cos(self.w() * t) * sin(self.w() * t - self.phi) )
